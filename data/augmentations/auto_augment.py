...
def level_to_arg(hparams):
    def _null_func(level):
        return ()
    
    def _posterize(level):
        return (int((level / _MAX_LEVEL) * 4), )
    
    def _solarize(level):
        return (int((level / _MAX_LEVEL) * 256), )
    
    def _solarize_add(level):
        return (int((level / _MAX_LEVEL) * 110), )
    
    def _cutout(level):
        return (int((level / _MAX_LEVEL) * hparams.cutout_const), )
    
    def _bbox_cutout(level):
         return _bbox_cutout_level_to_arg(level, hparams)
    
    def _trasnlate_bbox(level):
        return _translate_level_to_arg(level, hparams.translate_const)
    
    def _trasnlate_only_bbox(level):
        return  _translate_level_to_arg(level, hparams.translate_bbox_const)
    
    def _solarize_only_bbox(level):
        return (int((level / _MAX_LEVEL) * 256), )
    
    def _cutout_only_bbox(level):
        return (int((level / _MAX_LEVEL) * hparams.cutout_bbox_const), )
    
    return {
        'AutoContrast': _null_func,
        'Equalize': _null_func,
        ...
        'Cutout': _cutout,
        'BBox_Cutout':_bbox_cutout,
        'TranslateX_BBox': _trasnlate_bbox,
        'TranslateY_BBox': _trasnlate_bbox,
        ...
        'TranslateX_Only_BBoxes': _trasnlate_only_bbox,
        'TranslateY_Only_BBoxes': _trasnlate_only_bbox,
        'Flip_Only_BBoxes': _null_func,
        'Solarize_Only_BBoxes': _solarize_only_bbox,
        'Equalize_Only_BBoxes': _null_func,
        'Cutout_Only_BBoxes': _cutout_only_bbox,
    }
...
def test():
    ...
    def parser(serialized):
        ...
        image, boxes = distort_image_with_autoaugment(image, boxes, "v1")
    ...
    for images, boxes in dataset.take(20):
        ...
        boxes = boxes.numpy()[1] * np.array(list(image.shape[:2]) * 2)
