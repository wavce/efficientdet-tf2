...
class Head(object):
    def __init__(self, cfg, **kwargs):
        ...
        self.kernel_regularizer = (tf.keras.regularizers.l2(cfg.loss.weight_decay) 
                                   if cfg.loss.weight_decay is not None and cfg.loss.weight_decay > 0. else None)

    @property
    def min_level(self):
        return self.cfg.head.min_level
    
    @property
    def max_level(self):
        return self.cfg.head.max_level
    ...
    def _compute_losses_per_image(self, target_boxes, target_labels, predicted_boxes, predicted_labels, num_pos):
        with tf.name_scope("compute_losses_per_image"):
            target_labels = tf.cond(tf.greater(self.num_classes, 1), 
                                    lambda: tf.one_hot(tf.cast(target_labels, tf.int32), self.num_classes),
                                    lambda: tf.cast(tf.expand_dims(target_labels, -1), tf.float32))
            label_loss = self.label_loss_func(target_labels, predicted_labels)
            bbox_loss = self.bbox_loss_func(target_boxes, predicted_boxes)
    ...
    def compute_losses(self, outputs, image_info):
        with tf.name_scope("compute_losses"):
            ...
            for i in tf.range(tf.shape(target_boxes)[0]):
                ...
                anchors = tf.gather(total_anchors[i], sampling_result["positive_indices"])
                t_boxes = sampling_result["target_boxes"]
                p_boxes = tf.gather(predicted_boxes[i], sampling_result["positive_indices"])
                if self._use_iou_loss:
                    p_boxes = self.delta2box(anchors, p_boxes)
                else:
                    t_boxes = self.box2delta(anchors, t_boxes)
                b_loss, l_loss = self._compute_losses_per_image(
                    t_boxes, 
                    sampling_result["target_labels"],
                    p_boxes,
                    tf.gather(predicted_labels[i], sampling_result["label_indices"]),
